...
if __name__ == '__main__':
    ...
    parser.add_argument("-d", "--data_mode", help="use which database, [casia, vgg, ms1m, retina, ms1mr]",
                        default='ms1m', type=str)
    ...
    parser.add_argument("-head", "--head", help="head type, ['Softmax', 'ArcFace', 'CosFace', 'SFaceLoss']",
                        default='ArcFace', type=str)
    parser.add_argument("-t", "--target", help="verification targets", default='lfw,talfw,calfw,cplfw,cfp_fp,agedb_30',
                        type=str)
    ...
    trainloader = torch.utils.data.DataLoader(dataset, batch_size=BATCH_SIZE, shuffle=True, num_workers=len(GPU_ID),
                                              drop_last=True)
    ...
    for epoch in range(NUM_EPOCH):  # start training process
        ...
        for inputs, labels in iter(trainloader):
            ...
            if ((batch + 1) % DISP_FREQ == 0) and batch != 0:
                ...
                losses = AverageMeter()
                top1 = AverageMeter()
            # perform validation & save checkpoints (buffer for visualization)
